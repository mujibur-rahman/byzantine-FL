...
# ── Kappa scoring ────────────────────────────────────────────────────────────
def compute_kappa(model_params_i, global_params, X_val, scaler):
    """Kappa between client-updated model predictions and global model predictions."""
    n_feat = len(global_params) - 1

    m_i = FLModel(n_feat)
    m_i.set_params(model_params_i)

    m_g = FLModel(n_feat)
    m_g.set_params(global_params)
    ...
    if len(np.unique(pred_i)) < 2 or len(np.unique(pred_g)) < 2:
        return 0.0
    return cohen_kappa_score(pred_g, pred_i)
